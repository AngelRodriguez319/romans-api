...
sudo docker-compose -f bin/docker/docker-compose.yml run numeral_roman_django /bin/sh -c "chmod +x bin/*.sh;chmod +x bin/docker/*.sh"

echo "== Creating .env.devs"
sudo docker-compose -f bin/docker/docker-compose.yml run numeral_roman_django /bin/sh -c  "bin/docker/env-dev.sh $DJANGO_PORT $POSTGRES_PORT $REDIS_PORT $SERVER_URL $CLIENT_URL"

echo "== Starting services"
sudo docker-compose -f bin/docker/docker-compose.yml up -d

echo "== Executing db update (make & run migrations)"
sudo docker-compose -f bin/docker/docker-compose.yml exec numeral_roman_django /bin/sh -c "bin/docker/update.sh"

echo "== Loading dev fixtures (admin)"
sudo docker-compose -f bin/docker/docker-compose.yml exec numeral_roman_django /bin/sh -c "python manage.py loaddata bin/docker/fixtures-dev.yaml"

echo "== Generating docs"
sudo docker-compose -f bin/docker/docker-compose.yml exec numeral_roman_django /bin/sh -c "sphinx-build -E -b html ./seed/docs ./.data/docs"
